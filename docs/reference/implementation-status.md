...
| Provider Connections (P2, final closure; post-PP1 hardening) | implemented | Runtime-owned provider projections through the STRUCTURED OpenCode Server API (opencode serve: GET /provider, /provider/auth, OAuth authorize/callback — TUI output never parsed, auth file never read) + `opencode models`; Codex login status; Claude auth status; model_sources is the GENERIC OpenAI-compatible catalog endpoint configuration only (advanced surface, credential env REFERENCES, bounded on-demand catalogs never persisted); 9Router is NOT a first-class concept (UX inspiration only; an existing instance connects as a generic endpoint); audited source commands run inside the runCommand transaction (M10 atomicity: authority + OperatorAction + outcome commit together); frontend parses the real command envelope with strict guards (never optimistic); no routing/fallback/quota/rotation/alias logic anywhere; post-PP1: idempotent Begin BEFORE external side effects — sequentially AND concurrently (per-target process-local mutex keyed by connectionId+revision+providerId; `Promise.all` duplicate Begins produce EXACTLY one management session and one authorize, incompatible concurrent method loses with AUTH_FLOW_CONFLICT before any authorize), expected method type+label fingerprint revalidated against the fresh snapshot (reorder fails AUTH_METHOD_INVALID with zero authorize), browser-reload resume via bounded non-secret GET /provider-discovery/auth-flows/active proxied by the REAL Gateway route (cross-layer integration boots the built Gateway process against the real controller/service), OAuth flow authority fenced to the CURRENT non-revoked RuntimeConnection revision (Begin mutex keyed connectionId+providerId with authoritative revision resolved INSIDE the lock; stale-revision flows evicted session-closed no-callback; resume returns nothing for revoked/revised connections; Complete fails CONNECTION_REVOKED/AUTH_FLOW_STALE with callback count 0), truthful run picker (AUTH_REQUIRED "Sign in on Runtimes", UNAVAILABLE reason, DEGRADED warned-but-selectable, REVOKED never), reconnect allocation skips ANY occupied candidate id |
...
| Workspace execution / isolation (PP1)                    | implemented | Pivot invariant 1: the run's workspace determines every runtime child's cwd via the reserved `metadata.tenvyr.executionWorkspace` member; host validates path containment inside EXECUTOR_HOST_ALLOWED_ROOT (realpath trusted-code-only checks — spawn uses the real path, no traversal/symlink bypass); durable workspace_executions lease lifecycle (ALLOCATING→READY→IN_USE→PRESERVED→REMOVED/TRANSFERRED, fail-closed reconciliation, UNIQUE ownerRunId); shared | git-worktree modes; preservation-first cleanup (no `--force`); Safe Release uses tri-state worktree evidence, exact `EXECUTING` claim tokens, atomic terminal lock retirement, truthful stale recovery, and orphan-lock protection (see status JSON); Capsule/Workbench execution-workspace projection; no sandbox, no remote clones, no auto merge/push |
| Attention queue (PP1)                                     | implemented | Exception-driven pure READ projection (no durable state transition, no filesystem mutation); HUMAN_APPROVAL_REQUIRED / RUN_FAILED / LIMIT_REACHED / WORKSPACE_REQUIRES_ATTENTION over durable authority rows; deterministic ids (no polling duplicates); resolution only through existing authority commands; terminal IN_USE→PRESERVED is authoritative via run completion, not lazy polling; GET /workbench/attention + /attention page + dashboard NEEDS YOU; no attention table, no second authority system |
